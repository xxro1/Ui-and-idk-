...
ToggleButton.Position = UDim2.new(1, -55, 0.5, -22.5)
...
ButtonStroke.Color = Color3.fromRGB(255, 100, 150)
...
local function createTab(name, pos, width, color)
    local btn = Instance.new("TextButton")
    btn.Parent = TabBar
    btn.Name = name.."Tab"
    btn.Size = UDim2.new(0, width, 1, 0)
    btn.Position = UDim2.new(0, pos, 0, 0)
    btn.BackgroundColor3 = color or Color3.fromRGB(30, 25, 35)
    btn.Text = name
...
-- FLEX TABS NAMES
local OGTab = createTab(" ORBIT", 0, 90, Color3.fromRGB(255, 50, 100))
local VoidTab = createTab(" VOID", 95, 80, Color3.fromRGB(100, 80, 255))
local EvasionTab = createTab(" EVADE", 180, 80, Color3.fromRGB(50, 200, 150))
local AdvancedTab = createTab(" ADV", 265, 70, Color3.fromRGB(200, 100, 255))
...
function createToggle(parent, title, setting, order)
    ...
    toggle.BackgroundColor3 = UIConfig[setting] and Color3.fromRGB(255, 80, 120) or Color3.fromRGB(50, 45, 60)
    toggle.Text = ""
    ...
    toggle.MouseButton1Click:Connect(function()
        ...
        TweenService:Create(toggle, TweenInfo.new(0.2), {
            BackgroundColor3 = UIConfig[setting] and Color3.fromRGB(255, 80, 120) or Color3.fromRGB(50, 45, 60)
        }):Play()
        
        TweenService:Create(toggleCircle, TweenInfo.new(0.2), {
            Position = UIConfig[setting] and UDim2.new(1, -20, 0.5, -8) or UDim2.new(0, 4, 0.5, -8)
        }):Play()
    end)
...
function createSlider(parent, title, setting, min, max, suffix, order)
    ...
    local label = Instance.new("TextLabel")
    label.Parent = frame
    label.Size = UDim2.new(0, 160, 0, 18)
    ...
    sliderBg.BackgroundColor3 = Color3.fromRGB(50, 45, 60)
    ...
    dragButton.Position = UDim2.new((UIConfig[setting] - min) / (max - min), -9, 0.5, -9)
    ...
    local function updateSlider(input)
        ...
        dragButton.Position = UDim2.new(percent, -9, 0.5, -9)
...
function createDropdown(parent, title, setting, options, order)
    ...
    dropdown.BackgroundColor3 = Color3.fromRGB(40, 35, 50)
    ...
    dropdownCorner.CornerRadius = UDim.new(0, 30)
    ...
    arrow.TextSize = 12
    arrow.TextColor3 = Color3.fromRGB(255, 80, 120)
...
-- ORIGINAL ORBIT TAB CONTENT (FROM YOUR PIC)
local function loadOriginalOrbitTab()
    clearContent()
    local order = 0
    ...
    statusFrame.LayoutOrder = order; order = order + 1
    
    local statusDot = Instance.new("Frame")
    statusDot.Parent = statusFrame
    statusDot.Size = UDim2.new(0, 16, 0, 16)
    statusDot.Position = UDim2.new(0, 10, 0.5, -8)
    statusDot.BackgroundColor3 = OrbitEnabled and Color3.fromRGB(0, 255, 100) or Color3.fromRGB(255, 60, 60)
    ...
    levelFrame.LayoutOrder = order; order = order + 1
    ...
    levelText.TextColor3 = Color3.fromRGB(255, 100, 150)
    ...
    orbitButton.Size = UDim2.new(1, -20, 0, 55)
    orbitButton.Position = UDim2.new(0, 10, 0, 0)
    orbitButton.BackgroundColor3 = OrbitEnabled and Color3.fromRGB(255, 80, 120) or Color3.fromRGB(40, 35, 50)
    orbitButton.Text = " ORBIT AURA : " .. (OrbitEnabled and "ON" or "OFF")
    orbitButton.Font = Enum.Font.GothamBlack
    ...
    orbitButton.LayoutOrder = order; order = order + 1
    ...
    orbitButton.MouseButton1Click:Connect(function()
        ...
        TweenService:Create(orbitButton, TweenInfo.new(0.25), {
            BackgroundColor3 = OrbitEnabled and Color3.fromRGB(255, 80, 120) or Color3.fromRGB(40, 35, 50)
        }):Play()
    end)
end
...
local function updateTabColors()
    local tabs = {OGTab, VoidTab, EvasionTab, AdvancedTab}
    local tabNames = {" ORBIT", " VOID", " EVADE", " ADV"}
    
    for i, tab in ipairs(tabs) do
        if currentTab == tabNames[i] then
            tab.BackgroundColor3 = Color3.fromRGB(255, 80, 120)
            tab.TextColor3 = Color3.new(1, 1, 1)
        else
            tab.BackgroundColor3 = Color3.fromRGB(30, 25, 35)
            tab.TextColor3 = Color3.fromRGB(180, 180, 220)
        end
    end
end

OGTab.MouseButton1Click:Connect(function()
    currentTab = " ORBIT"
    updateTabColors()
    loadOriginalOrbitTab()
end)
