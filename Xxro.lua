...
local OrbitTab = createTab("ORBIT", 0, 85)
local EvadeTab = createTab("EVADE", 95, 85)
local AdvTab = createTab("ADV", 190, 80)
...
function createToggle(parent, title, setting, order)
    ...
    toggle.MouseButton1Click:Connect(function()
        ...
        TweenService:Create(toggle, TweenInfo.new(0.2), {
            BackgroundColor3 = UIConfig[setting] and Color3.fromRGB(150, 150, 255) or Color3.fromRGB(50, 50, 60)
        }):Play()
...
function createDropdown(parent, title, setting, options, order)
    ...
    arrow.TextSize = 10
    arrow.TextColor3 = Color3.fromRGB(150, 150, 255)
...
local function loadOrbitTab()
    ...
    orbitButton.Size = UDim2.new(1, -20, 0, 50)
...
local function loadAdvTab()
    ...
    createToggle(ContentFrame, "Force Vertical", "ForceVerticalEvasion", order); order = order + 1
    createSlider(ContentFrame, "Gravity Strength", "GravityWellStrength", 0, 10000, " M", order); order = order + 1
end
...
local function updateTabColors()
    local tabs = {OrbitTab, EvadeTab, AdvTab}
    local tabNames = {"ORBIT", "EVADE", "ADV"}
    
    for i, tab in ipairs(tabs) do
        if currentTab == tabNames[i] then
            tab.BackgroundColor3 = Color3.fromRGB(150, 150, 255)
...
print("✅ xxro beta loaded")
print("✅ ORBIT tab - your original orbit")
print("✅ EVADE tab - evasion settings")
print("✅ ADV tab - advanced settings")
